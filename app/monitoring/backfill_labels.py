...
def pick_latest_manifest() -> Path:
    """Pick the newest generated stream manifest."""
...
def derive_label(
    ...
    if policy == "scenario_default":
        ...
        if flip_prob is None:
            raise ValueError(
                "flip_prob must be provided for custom_flip policy"
            )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.label_policy == "custom_flip":
        if args.flip_prob is None:
            raise ValueError(
                "flip_prob must be provided when label_policy=custom_flip"
            )
...
def post_labels_batch(
    api_url: str, labels: list[dict[str, Any]], timeout: float
) -> dict[str, Any]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Backfill delayed ground-truth labels"
    )
    ...
    parser.add_argument(
        "--api-url", type=str, default=f"http://localhost:{settings.api_port}"
    )
    ...
    label_ts = datetime.now(UTC) - timedelta(hours=float(args.delay_hours))
